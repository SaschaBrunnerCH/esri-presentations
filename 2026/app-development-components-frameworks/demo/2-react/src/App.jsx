  ...
  const handleMapReadyChange = async (event) => {
    ...
    const newSelectionHandler = mapElement.view.selectionManager.on(
      ...
      async () => {
        ...
        const results =
          await mapElement.view.selectionManager.getSelectedFeatures(
            [layer],
            { returnGeometry: true, outFields: ["*"] },
            "layer", // Fetch full trail geometry even when it is not loaded in the view.
          );
